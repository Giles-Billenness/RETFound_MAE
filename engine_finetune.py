...
def misc_measures(confusion_matrix):
    ...
    for i in range(1, confusion_matrix.shape[0]):
        ...
        mcc = (cm1[0, 0]*cm1[1, 1]-cm1[0, 1]*cm1[1, 0])/np.sqrt((cm1[0, 0]+cm1[0, 1])
                                                                * (cm1[0, 0]+cm1[1, 0])*(cm1[1, 1]+cm1[1, 0])*(cm1[1, 1]+cm1[0, 1]))
        mcc_.append(mcc)
...
def evaluate(data_loader, model, device, task, outputdir, epoch, mode, num_class):
    ...
    for batch in metric_logger.log_every(data_loader, 10, header):
        ...
        risk_factors = risk_factors.to(device, non_blocking=True)
        true_label = F.one_hot(target.to(torch.int64), num_classes=num_class)

        # compute output
        with torch.cuda.amp.autocast():
            output = model(images, risk_factors)
            ...
            true_label_decode_list.extend(
                true_label_decode.cpu().detach().numpy())
    ...
    acc, sensitivity, specificity, precision, G, F1, mcc = misc_measures(
        confusion_matrix)

    auc_roc = roc_auc_score(true_label_onehot_list,
                            prediction_list, multi_class='ovr', average='macro')
    auc_pr = average_precision_score(
        true_label_onehot_list, prediction_list, average='macro')

    co_kap = cohen_kappa(torch.from_numpy(prediction_decode_list), torch.from_numpy(
        true_label_decode_list), task="multiclass", num_classes=num_class).item()

    metric_logger.synchronize_between_processes()

    print('Sklearn Metrics - Acc: {:.4f} AUC-roc: {:.4f} AUC-pr: {:.4f} F1-score: {:.4f} MCC: {:.4f} Cohens kappa: {:.4f}'.format(
        acc, auc_roc, auc_pr, F1, mcc, co_kap))
    results_path = outputdir/task/str(task+f'_metrics_{mode}.csv')
    with open(results_path, mode='a', newline='', encoding='utf8') as cfa:
        wf = csv.writer(cfa)
        data2 = [[acc, sensitivity, specificity, precision,
                  auc_roc, auc_pr, F1, mcc, co_kap, metric_logger.loss]]
        for i in data2:
            wf.writerow(i)

    if mode == 'test':
        cm = ConfusionMatrix(actual_vector=true_label_decode_list,
                             predict_vector=prediction_decode_list)
        cm.plot(cmap=plt.cm.Blues, number_label=True,
                normalized=True, plot_lib="matplotlib")
        plt.savefig(outputdir/task/'confusion_matrix_test.jpg',
